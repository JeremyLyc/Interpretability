...
def main():
    ...
    for graph_idx in graph_idxs:
        ...
        if prog_args.plot:
            ...
            if org_pred == extracted_pred == ours_pred == graph_label:
                prefix = 'correct_'
            else:
                prefix = 'wrong_'
        ...
        pred_prob += [[graph_idx] + list(org_p) + list(extracted_p) + list(ours_p)]
        if np.inf in [log_odd(org_p[graph_label]), log_odd(ours_p[graph_label]),
                        log_odd(org_p[org_pred]), log_odd(ours_p[org_pred])]:
    ...
    merged_loss = np.stack([valid_graph_idxs, org_losses, extracted_losses, ours_losses], axis=1)
    np.savetxt("log/%s/loss.csv"%(prog_args.test_out), merged_loss, delimiter=",")
    merged_correct = np.stack([valid_graph_idxs, org_corrects, extracted_corrects, ours_corrects], axis=1)
    np.savetxt("log/%s/acc.csv"%(prog_args.test_out), merged_correct, delimiter=",")
    merged_odd = np.stack([valid_graph_idxs, our_ground_truth_odd, our_pred_label_odd])
